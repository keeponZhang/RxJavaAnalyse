                ...
                Observable.create(new Observable.OnSubscribe<Student>() {
                    ...
                    public void call(Subscriber<? super Student> subscriber) {
                        try {
                            ...
                            Log.e("TAG", "Rx1Sample2Activity call 开始加载网络数据:" +
                                    System.currentTimeMillis());
//                     Student keepon_from_net = new Student("keepon from net");
                            ...
                            subscriber.onNext(keepon_from_net);
                            subscriber.onCompleted();
                            Log.e("TAG", "Rx1Sample2Activity call 加载网络数据结束:" +
                                    System.currentTimeMillis());
                        } catch (Exception e) {
    ...
    public void merge3(View view) {
        ...
        //merge括号里是一个Observable(OnSubscribeFromIterable)，merge会用到lift，又会new一个Observable,
        //merge里面的元素会通过from，变成要发射的Observable
        Observable.from(Arrays.asList(getDataFromNet(), getDataFromLocal2()))
                .lift(new OperatorMerge<Student>())
                .subscribe(new Observer<Student>() {
                    @Override
                    public void onCompleted() {
