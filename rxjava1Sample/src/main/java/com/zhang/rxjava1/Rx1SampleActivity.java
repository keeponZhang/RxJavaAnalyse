...
public class Rx1SampleActivity extends AppCompatActivity {
    ...
    public void map2(View view) {
        //subscribe指出触发一层层订阅
        Observable.just("http://ep.dzb.ciwong.com/rep/new/4055.jpg")// ObservableJust
                .lift(new OperatorMap<String, Bitmap>(new Func1<String, Bitmap>() {
                    @Override
                    public Bitmap call(String urlPath) {
                        try {
                            return BitmapFactory
    ...
    public void flatMap(View view) {
        ...
        Observable.from(new School().getClasses())
            ...
            public void onNext(Group group) {
                Log.e(TAG, "Observable flatMap onNext:" + group);
            }
        });
    }

    // flatMap2最基本原理
    public void flatMap22(View view) {
        //OperatorMerge接收的是observable
        Observable.create(new Observable.OnSubscribe<Integer>() {
            @Override
            public void call(Subscriber<? super Integer> subscriber) {
                Log.w("TAG", "Rx1SampleActivity Observable call 发送东西啦  发送1:");
                subscriber.onNext(1);
                Log.w("TAG", "Rx1SampleActivity  Observable call 发送东西啦  发送2:");
                subscriber.onNext(2);

            }
        }).lift(new OperatorMap<Integer, Observable<String>>(
                ...
                })).lift(new OperatorMerge<String>()).subscribe(new Subscriber<String>() {
            @Override
            public void onCompleted() {
            ...
            @Override
            public void onNext(String s) {
                Log.e(TAG, "Observable flatMap22 onNext:" + s);
            }
        });
        ;
    ...
    public void flatMap2(View view) {
        Observable.just(1, 2)
                ...
                .flatMap(new Func1<Integer, Observable<String>>() {
                    @Override
                    public Observable<String> call(Integer integer) {
                        Log.d(TAG, "ObservableflatMap2 call:" + integer);
                        final List<String> list = new ArrayList<>();
                        for (int i = 0; i < 3; i++) {
            ...
            public void run() {
                getObservableCreat()
                        ...
                        .doOnSubscribe(new Action0() {
                            ...
                            public void call() {
                                Log.e(TAG,
                                        "doOnSubscribe call:" + Thread.currentThread().getName());
                            }
                        })
