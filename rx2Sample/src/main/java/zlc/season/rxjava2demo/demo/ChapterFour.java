...
public class ChapterFour {
    ...
    public static void demo3() {
        Observable<Integer> observable1 = Observable.create(new ObservableOnSubscribe<Integer>() {
            @Override
            public void subscribe(ObservableEmitter<Integer> emitter) throws Exception {
                Log.d(TAG, "ZipCoordinator emit 1");
                emitter.onNext(1);
                Thread.sleep(1000);

                Log.d(TAG, "ZipCoordinator emit 2");
                emitter.onNext(2);
                Thread.sleep(1000);

                Log.d(TAG, "ZipCoordinator emit 3");
                emitter.onNext(3);
                Thread.sleep(1000);

                Log.d(TAG, "ZipCoordinator emit 4");
                emitter.onNext(4);
                Thread.sleep(1000);

                Log.d(TAG, "ZipCoordinator emit complete1");
                emitter.onComplete();
            }
        ...
        Observable<String> observable2 = Observable.create(new ObservableOnSubscribe<String>() {
            @Override
            public void subscribe(ObservableEmitter<String> emitter) throws Exception {
                Log.d(TAG, "ZipCoordinator emit A");
                emitter.onNext("A");
                Thread.sleep(1000);

                Log.d(TAG, " ZipCoordinator emit B");
                emitter.onNext("B");
                Thread.sleep(1000);

                Log.d(TAG, " ZipCoordinator emit C");
                emitter.onNext("C");
                Thread.sleep(1000);

                Log.d(TAG, "ZipCoordinator emit complete2");
                emitter.onComplete();
            }
        }).subscribeOn(Schedulers.io());
        ...
        Observable.zip(observable1, observable2, new BiFunction<Integer, String, String>() {
            ...
            @Override
            public void onSubscribe(Disposable d) {
                Log.d(TAG, "ZipCoordinator onSubscribe");
            }

            @Override
            public void onNext(String value) {
                Log.d(TAG, "ZipCoordinator onNext: " + value);
            }

            @Override
            public void onError(Throwable e) {
                Log.d(TAG, "ZipCoordinator onError");
            }

            @Override
            public void onComplete() {
                Log.d(TAG, "ZipCoordinator onComplete");
            }
        });
    }
