...
public final class OnSubscribeRedo<T> implements OnSubscribe<T> {
    ...
    static final Func1<Observable<? extends Notification<?>>, Observable<?>> REDO_INIFINITE =
            new Func1<Observable<? extends Notification<?>>, Observable<?>>() {
                @Override
                public Observable<?> call(Observable<? extends Notification<?>> ts) {
                    System.out.println(getOnSubscribeRedoTag() +
                            "  1.1--------- REDO_INIFINITE  call------------》》》》》");
    ...
    public static final class RedoFinite
        ...
        @Override
        public Observable<?> call(Observable<? extends Notification<?>> ts) {
            Log.e("TAG", getOnSubscribeRedoTag() +
                    " RedoFinite  0.6 (准备lift 2次，map，OperatorDematerialize)---------  map" +
                    "  " +
    ...
    public static final class RetryWithPredicate implements
            Func1<Observable<? extends Notification<?>>, Observable<? extends Notification<?>>> {
        ...
        @Override
        public Observable<? extends Notification<?>> call(
                Observable<? extends Notification<?>> ts) {
            return ts.scan(Notification.createOnNext(0),
                    new Func2<Notification<Integer>, Notification<?>, Notification<Integer>>() {
                        @SuppressWarnings("unchecked")
                        @Override
                        public Notification<Integer> call(Notification<Integer> n,
                                                          Notification<?> term) {
    ...
    public static <T> Observable<T> retry(Observable<T> source,
        ...
        return create(new OnSubscribeRedo<T>(source, notificationHandler, true, false,
    ...
    public static <T> Observable<T> repeat(Observable<T> source,
                                           Func1<? super Observable<? extends Notification<?>>, ? extends Observable<?>> notificationHandler) {
        return create(new OnSubscribeRedo<T>(source, notificationHandler, false, true,
                Schedulers.trampoline()));
    }
    ...
    public static <T> Observable<T> redo(Observable<T> source,
                                         Func1<? super Observable<? extends Notification<?>>, ? extends Observable<?>> notificationHandler,
                                         Scheduler scheduler) {
    ...
    private final Func1<? super Observable<? extends Notification<?>>, ? extends Observable<?>>
            controlHandlerFunction;
    private boolean stopOnComplete;
    private boolean stopOnError;
    private final Scheduler scheduler;

    private OnSubscribeRedo(Observable<T> source,
                            Func1<? super Observable<? extends Notification<?>>, ? extends Observable<?>> f,
                            boolean stopOnComplete, boolean stopOnError,
                            Scheduler scheduler) {
    ...
    public void call(final Subscriber<? super T> child) {
            ...
            public void call() {
                ...
                Subscriber<T> terminalDelegatingSubscriber = new Subscriber<T>() {
                    ...
                    public void onCompleted() {
                        unsubscribe();
                        Log.e("TAG", getOnSubscribeRedoTag() + " 真正的subscriber发送完成，回调onCompleted" +
                                "反复，准备repeat啦啦啦 " +
                                " " +
                                "调用(PublishSubject)terminals.onNext,参数（Notification类型）:" +
                    ...
                    public void onNext(T v) {
                        ...
                        Log.i("TAG",
                                getOnSubscribeRedoTag() + " terminalDelegatingSubscriber 1.5 " +
                                        "接送到source 发送来的事件 onNext=" + v);
                        //调用该方法把事件发给下层的订阅者
                        child.onNext(v);
                    }
                ...
                Log.e("TAG", getOnSubscribeRedoTag() +
                        "  subscribeToSource ---soure订阅开始喽 source.unsafeSubscribe(terminalDelegatingSubscriber)" +
        ...
        Observable<Notification<?>> terminalslift =
                terminals.lift(new Operator<Notification<?>, Notification<?>>() {
                    //下面call方法是Operator的call方法，返回另一个Subscriber
                    @Override
                    public Subscriber<? super Notification<?>> call(
                            final Subscriber<? super Notification<?>> filteredTerminals) {
                        //启动订阅后，会OnSubscribe的call方法，一层一层上去，这里就是其中几个lift的一层
                        Log.w("TAG", getOnSubscribeRedoTag() +
                                "  1.2--------- 调用call方法 restarts Operator " +
                                "call------------filteredTerminals 》》》》》" + filteredTerminals +
                                " subsriber= " + this);
                        return new Subscriber<Notification<?>>(filteredTerminals) {
                            ...
                            public void onError(Throwable e) {
                                Log.w("TAG", getOnSubscribeRedoTag() +
                                        "  Notification filteredTerminals.onError(e)" +
                                        ":" + e);
                            ...
                            public void onNext(Notification<?> t) {
                                ...
                                Log.w("TAG",
                                        getOnSubscribeRedoTag() + " 1.95 重点 terminalslift call   " +
                                                "onNext 数据类型 t:" + t);
                                if (t.isOnCompleted() && stopOnComplete) {
                                    Log.d("TAG", getOnSubscribeRedoTag() +
                                            "  1.98 onNext   Subscriber " +
                                            "terminals.lift onNext <<  if stopOnComplete >>" +
                                            this + "  Notification.getKind=" + t.getKind());
                                    child.onCompleted();
                                } else if (t.isOnError() && stopOnError) {
                                    Log.d("TAG",
                                            getOnSubscribeRedoTag() + " onNext   1.98 Subscriber " +
                                                    "terminals.lift onNext << else if stopOnError >>" +
                                                    this + "  Notification.getKind=" + t.getKind());
                                    child.onError(t.getThrowable());
                                } else {
                                    Log.d("TAG", getOnSubscribeRedoTag() +
                                            " onNext  1.98   Subscriber " +
                                            "terminals.lift onNext << else  >>" + this +
                                            "  Notification.getKind=" + t.getKind());
                                    isLocked.set(false);
                                    //filteredTerminals 这个是下层传上来的，lift里面创建的subscriber
        ...
        Log.e("TAG", getOnSubscribeRedoTag() +
                " call   0.8 terminals.lift fun1.call(controlHandlerFunction.call)方法调用完毕:");
        ...
        worker.schedule(new Action0() {
            ...
            public void call() {
                ...
                Log.i("TAG", getOnSubscribeRedoTag() +
                        "  0.9--------- worker.schedule call------------》》》》》");
                Subscriber<Object> workerSubscriber = new Subscriber<Object>(child) {
                    @Override
                    public void onCompleted() {
                        System.out
                                .println(getOnSubscribeRedoTag() + " workerSubscriber onCompleted");
                        child.onCompleted();
                    }
                    ...
                    public void onNext(Object t) {
                        ...
                        Log.e("TAG", getOnSubscribeRedoTag() + "  ---------workerSubscriber " +
                                "这里是重新发送最底层的观察者" +
                                " " +
                                "重新发射" +
                                " " +
                                "worker.schedule   " +
                                "restarts onNext==" + t + " subscriber=" + this + " ---------" +
                                "  isLockedBoolean=" + isLockedBoolean + " unsubscribed=" +
                                unsubscribed);
                    ...
                    public void setProducer(Producer producer) {
                        System.out.println(
                                getOnSubscribeRedoTag() + "   worker.schedule setProducer ");
                        producer.request(Long.MAX_VALUE);
                    }
                };
                Log.e("TAG",
                        getOnSubscribeRedoTag() + " 1 (负责重复发送的Observable开始订阅，实际发送需要触发terminals" +
                                ".onNext方法(因为最顶层Observable是PublishObservable))" +
                                "---------" +
                                "  restarts" +
                                ".unsafeSubscribe" +
                                "(workerSubscriber) restarts==" + restarts + " workerSubscriber=" +
                                workerSubscriber + " ---------");
